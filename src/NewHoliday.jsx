// eslint-disable-next-line no-unused-vars
import React from "react";
import { RxMoon } from "react-icons/rx";
const NewHoliday = () => {
  return (
    <div className="fixed top-4 left-0 w-screen">
    <div className="relative flex xxxs:justify-center sm:justify-between   ">
      <div className=" w-fit sm:ml-4   "> 
      <a href="https://Mybabb.com"  className="topParagraphWrapper   
               font-Aclonica-Regular" >
        
          <p className="topParagraph   whitespace-nowrap 
               text-3xl sm:text-4xl md:text-5xl lg:text-6xl   ">
          <span className='text-blue-200 inline-block'  >Moon&nbsp;Day&nbsp;</span>
          <span className='text-blue-200 inline-block mx-2' ><RxMoon size={29} /></span>
           </p> 
          
          
       
      </a>
      </div>

      <div className="demoWrapper"> 
      <a href="https://Mybabb.com">
                <div className="  relative    min-w-[225px] lg:min-w-[335px] flex  ">
                <p
                  id="demo"
                  className="m-auto relative flex ml-8 md:ml-0  justify-start     w-full text-lg sm:text-lg md:text-2xl lg:text-4xl
                   "
                ></p>
                </div>
      </a> 
      </div>
      </div>




    </div>
  );
};

export default NewHoliday;
